...
def format_temperature_data(data: dict) -> tuple:
    ...
    time_data = [datetime.datetime.fromtimestamp(timestamp) for timestamp in time_data]
...
def group_weather_data(weather_data: list) -> dict:
    ...
    for date, temperature, humidity, description in weather_data:
        ...
        grouped_weather_data[date].append((temperature, humidity, description))
    ...
    try:
        ...
        if data:
            ...
            for date, weather_list in grouped_weather_data.items():
                average_temperature = sum([t for t, h, d in weather_list]) / len(weather_list)
                average_humidity = sum([h for t, h, d in weather_list]) / len(weather_list)
